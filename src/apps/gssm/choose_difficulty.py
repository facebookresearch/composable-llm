# %%
import yaml
from apps.gssm.difficulty import read_results, DifficultyEstimationConfig
...
def main() -> None:
    ...
    for name, config in all_configs.items():
        ...
        plt.scatter([x for x in hmm_estimate.values()], gzip_estimate.values(), alpha=.3, label=name)
        for (alphas, h_estimate), gzip_estimate in zip(hmm_estimate.items(), gzip_estimate.values()):
            # plt.text(h_estimate, gzip_estimate, f"{alphas[0]:.2f},{alphas[1]:.2f}", ha="center", va="center", font
            plt.annotate(f"{alphas[1]:.2e}", xy=(h_estimate, gzip_estimate), xytext=(-10, 10),
                         textcoords="offset points", ha="right", va="bottom", arrowprops=dict(arrowstyle="->"), fontsize=5)
    plt.xlabel("H estimate")
    plt.ylabel("compression rate")
    plt.legend()
    plt.savefig(f"compr_vs_h.png")
