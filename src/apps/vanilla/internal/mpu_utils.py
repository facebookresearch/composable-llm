...
def initialize_model_parallel(
    ...
    *,
    ...
    ensure_divisibility(
        world_size, model_parallel_size * pipeline_length * context_parallel_size
    )
    ...
    if torch.distributed.get_rank() == 0:
        ...
        print(
            "> initializing context parallel with size {}".format(context_parallel_size)
        )
    ...
    groups = torch.LongTensor(range(world_size)).reshape(
        data_parallel_size, pipeline_length, context_parallel_size, model_parallel_size
    )
        ...
        for j in range(context_parallel_size):
            for k in range(model_parallel_size):
                group = torch.distributed.new_group(
                    groups[:, i, j, k].tolist(), backend=ddp_backend, timeout=timeout
                )
...
def all_gather(x: torch.Tensor) -> torch.Tensor:
    ...
    torch.distributed.all_gather(gather, x, group=get_model_parallel_group())
...
def all_reduce(x: torch.Tensor):
    if get_model_parallel_world_size() > 1:
        # reduce with a sum
        torch.distributed.all_reduce(x, group=get_model_parallel_group())
