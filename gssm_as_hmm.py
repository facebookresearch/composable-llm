...
class HMM:
    ...
    def __init__(self, config=None, top_node=None, random_seed=100):
        ...
        self.transitions = {
            node: self._format_transition(node) for _, node in self.topo_order
        }
    ...
    def one_hot_product_state_fast(self):
        node_order = [
            (name, node) for name, node in self.topo_order if not node.observed
        ]
        einsum_str = (
            "B"
            + ",B".join(HMM.SYM_IN[: len(node_order)])
    ...
    def einsum_full_prod_str_fast(self):
        ...
        """
        in_str = "".join(
            self._node_sym_in(n) for _, n in self.topo_order if not n.observed
        )
        out_str = "".join(
            self._node_sym_out(n) for _, n in self.topo_order if not n.observed
        )
    ...
    def forward_algorithm(
        ...
        forward_probs = torch.zeros(
            (num_states, T, B), device=device, dtype=torch.float32
        )

        forward_probs[:, 0, :] = log_pi[
            :, None
        ]  # + log_E[:, obs[0]] <- don't need this because we start the emission with the prior
    ...
    def forward_probs(
        self, observations: np.ndarray, device: str = "cuda", small_mem=False
    ):
        T, B = observations.shape
        observations = torch.as_tensor(observations, dtype=torch.int32, device=device)
        self._init_all_nodes(B)
        prior = torch.tensor(
            self.one_hot_product_state()[0], device=device, dtype=torch.float32
        )
        transition = torch.tensor(
            self.make_prod_transition(), device=device, dtype=torch.float32
        )
        emission = torch.tensor(
            self.get_p_emission(), device=device, dtype=torch.float32
        )
        # print("from fwd probs", transition.shape, emission.shape, prior.shape)
        return self.forward_algorithm(
            observations,
            transition.log(),
            emission.log(),
            prior.log(),
    ...
    def forward_probs_fast(
        ...
        self._init_all_nodes(B)
        prior = torch.tensor(
            self.one_hot_product_state_fast()[0], device=device, dtype=torch.float32
        )
        transition = torch.tensor(
            self.make_prod_transition_fast(), device=device, dtype=torch.float32
        )
        emission = torch.tensor(
            self.get_p_emission_fast(), device=device, dtype=torch.float32
        )
    ...
    def entropy_of_observations(
        ...
        if len([node for _, node in self.topo_order if node.mode == "context"]) != 0:
            ...
            if fast:
                _, log_xst = self.forward_probs_fast(
                    observations, device=device, small_mem=small_mem
                )
            else:
                _, log_xst = self.forward_probs(
                    observations, device=device, small_mem=small_mem
                )
        H_t = -log_xst.cpu()
        if final_entry_only:
    ...
    def forward_probs_ICL(
        self, observations: np.ndarray, N2: int = 300, verbose=False
    ) -> np.ndarray:
        # Second version
        ...
        for i in range(B):
            ...
            for j in range(N2):
                random_hmm.rng.bit_generator.state = np.random.default_rng(
                    self.random_seed + 10 + i * N2 + j
                ).bit_generator.state
                random_hmm.top_node.initialize(B)
                random_hmm.transitions = {
                    node: random_hmm._format_transition(node)
                    for _, node in random_hmm.topo_order
                }
                ...
                conditional_log_xst = (
                    conditional_log_xst[:, 0].cpu().numpy()
                )  # Now conditional_log_xst ~= [log(P(X_[1] | hmm)),...,log(P(X_[T]| hmm))]
                # [N2, T]
            ...
            logliks_of_the_observations.append(
                np.logaddexp.reduce(
                    np.array(conditional_logliks_of_the_observation), axis=0
                )
                - np.log(N2)
            )
