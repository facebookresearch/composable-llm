...
def initialize_nested_object(object_type: type[T], data: dict[str, Any], inplace: bool = True) -> T:
    ...
    if is_dataclass(object_type):
        ...
        for data_field in fields(object_type):
            ...
            if fname in data:
                ...
                print(f"Info: Field '{fname}' not found in {object_type}, using default")
        for fname in data:
            print(f"Field '{fname}' ignored when initializing {object_type}.")
    ...
    try:
        ...
        print(f"Initializing {object_type}:{data} without type checking.")
        return data
